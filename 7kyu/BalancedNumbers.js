...
function balancedNum(number) {
    ...
    console.log(numArr);
    if (numArr.length <= 2) console.log("Balanced");
    //get the middle
    let middle = Math.floor(numArr.length / 2);
    //get midd pos if length % 2 === 0 then mid -1 : mid;
    let middlePos = numArr.length % 2 === 0 ? middle - 1 : middle;
    let left = numArr.slice(0, middlePos).reduce((total, num) => total += num , 0);
    let right = numArr.slice(-middlePos).reduce((total, num) => total += num , 0);
    //left and right slice then reduce the array to get total of each 
    //if left  === to right "Balanced"
